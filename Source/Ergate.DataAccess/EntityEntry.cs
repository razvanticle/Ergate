namespace Ergate.DataAccess
{
    using System.Collections.Generic;
    using System.Data.Entity;
    using System.Data.Entity.Infrastructure;
    using System.Linq;

    sealed class EntityEntry<T> : IEntityEntry<T>
        where T : class
    {
        private readonly DbEntityEntry<T> entry;

        public EntityEntry(DbEntityEntry<T> entry)
        {
            this.entry = entry;
        }

        public T Entity
        {
            get { return this.entry.Entity; }
        }

        public EntityEntryState State
        {
            get { return (EntityEntryState)this.entry.State; }
            set { this.entry.State = (EntityState)value; }
        }

        public object GetOriginalValue(string propertyName)
        {
            return this.entry.OriginalValues[propertyName];
        }

        public object GetCurrentValue(string propertyName)
        {
            return this.entry.CurrentValues[propertyName];
        }

        public void SetOriginalValue(string propertyName, object value)
        {
            if (this.entry.OriginalValues.PropertyNames.Contains(propertyName))
                this.entry.OriginalValues[propertyName] = value;
        }

        public void Reload()
        {
            this.entry.Reload();
        }

        public IEnumerable<string> GetProperties()
        {
            return this.entry.CurrentValues.PropertyNames;
        }

        public IPropertyEntry Property(string name)
        {
            return new PropertyEntry(this.entry.Property(name));
        }

        private bool Equals(EntityEntry<T> other)
        {
            return Equals(this.entry, other.entry);
        }

        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj)) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj is EntityEntry<T> && this.Equals((EntityEntry<T>)obj);
        }

        public override int GetHashCode()
        {
            return (this.entry != null ? this.entry.GetHashCode() : 0);
        }
    }

    sealed class EntityEntry : IEntityEntry
    {
        private readonly DbEntityEntry entry;

        public EntityEntry(DbEntityEntry entry)
        {
            this.entry = entry;
        }

        public object Entity
        {
            get { return this.entry.Entity; }
        }

        public EntityEntryState State
        {
            get { return (EntityEntryState)this.entry.State; }
            set { this.entry.State = (EntityState)value; }
        }

        public object GetOriginalValue(string propertyName)
        {
            return this.entry.OriginalValues[propertyName];
        }

        public object GetCurrentValue(string propertyName)
        {
            return this.entry.CurrentValues[propertyName];
        }

        public IEntityEntry<T> Convert<T>() where T : class
        {
            return new EntityEntry<T>(this.entry.Cast<T>());
        }

        public void SetOriginalValue(string propertyName, object value)
        {
            if (this.entry.OriginalValues.PropertyNames.Contains(propertyName))
                this.entry.OriginalValues[propertyName] = value;
        }

        public void Reload()
        {
            this.entry.Reload();
        }

        public IEnumerable<string> GetProperties()
        {
            return this.entry.CurrentValues.PropertyNames;
        }

        public IPropertyEntry Property(string name)
        {
            return new PropertyEntry(this.entry.Property(name));
        }

        private bool Equals(EntityEntry other)
        {
            return Equals(this.entry, other.entry);
        }

        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj)) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj is EntityEntry && this.Equals((EntityEntry)obj);
        }

        public override int GetHashCode()
        {
            return (this.entry != null ? this.entry.GetHashCode() : 0);
        }
    }

    sealed class PropertyEntry : IPropertyEntry
    {
        private readonly DbPropertyEntry entry;

        public PropertyEntry(DbPropertyEntry entry)
        {
            this.entry = entry;
        }

        public string Name
        {
            get { return this.entry.Name; }
        }
        public object CurentValue
        {
            get { return this.entry.CurrentValue; }
            set { this.entry.CurrentValue = value; }
        }
        public object OriginalValue
        {
            get { return this.entry.OriginalValue; }
            set { this.entry.OriginalValue = value; }
        }
        public bool IsModified
        {
            get { return this.entry.IsModified; }
            set { this.entry.IsModified = value; }
        }
    }
}